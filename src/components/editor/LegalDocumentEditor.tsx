...
export const LegalDocumentEditor: React.FC = () => {
  ...
  const resolveDocContent = (rawContent?: string, variables?: Record<string, any>) => {
    ...
    if (trimmed === '{{__content__}}' || trimmed === '{{ __content__ }}' || trimmed === '' || trimmed.startsWith('<!-- WIZARD_GENERATED')) {
      ...
      return compileHouseAgreement(state);
    }
    return rawContent;
  };

  const initialContent = resolveDocContent(doc?.content, doc?.variables);
  ...
  const isHouseWizardDoc = contentHtml?.startsWith('<!DOCTYPE') || contentHtml?.startsWith('<!');

  useEffect(() => {
    if (doc) {
      const resolved = resolveDocContent(doc.content, doc.variables);
      setContentHtml(resolved);
      const isWizard = resolved?.startsWith('<!DOCTYPE') || resolved?.startsWith('<!');
      if (!isWizard && editorRef.current) {
        editorRef.current.innerHTML = resolved;
      }
      if (isWizard && iframeRef.current) {
        iframeRef.current.srcdoc = resolved;
      }
    }
  }, [doc?.id, doc?.content, doc?.variables]);
      ...
      <div className="flex-1 flex overflow-hidden relative">
        <div className="flex-1 overflow-y-auto overflow-x-hidden bg-[#d0d0d0] dark:bg-[#08090b]">
          {isHouseWizardDoc ? (
            ...
            <iframe
              ref={iframeRef}
              srcDoc={contentHtml || doc.content}
